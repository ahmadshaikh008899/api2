...
def update_soul_txt(public_url):
    ...
    print(f"New ngrok link saved in api.txt")
...
def update_vps_soul_txt(public_url):
    ...
    try:
        ...
        sftp = ssh.open_sftp()
        with sftp.open("api.py", "w") as file:  # Fixed syntax error here
            file.write(public_url)
        sftp.close()
        ssh.close()
        print("Updated api.py on VPS successfully.")
    except Exception as e:
        print(f"Failed to update api.py on VPS: {str(e)}")
